...
void RenderState::StateBlock::bindNoRestore()
{
    ...
    {
        if (_blendEnabled)
            GPRHI_ASSERT( GPRHI_Enable(GP_RHI_RS_BLEND) );
        else
            GPRHI_ASSERT( GPRHI_Disable(GP_RHI_RS_BLEND) );
    ...
    {
        if (_cullFaceEnabled)
            GPRHI_ASSERT( GPRHI_Enable(GP_RHI_RS_CULL_FACE) );
        else
            GPRHI_ASSERT( GPRHI_Disable(GP_RHI_RS_CULL_FACE) );
    ...
    if ((_bits & RS_DEPTH_TEST) && (_depthTestEnabled != _defaultState->_depthTestEnabled))
    {
        if (_depthTestEnabled)
            GPRHI_ASSERT( GPRHI_Enable(GP_RHI_RS_DEPTH_TEST) );
        else
            GPRHI_ASSERT( GPRHI_Disable(GP_RHI_RS_DEPTH_TEST) );
    ...
    {
        GPRHI_ASSERT( GPRHI_DepthMask(_depthWriteEnabled ? GP_TRUE : GP_FALSE) );
	...
	if ((_bits & RS_STENCIL_TEST) && (_stencilTestEnabled != _defaultState->_stencilTestEnabled))
    {
        if (_stencilTestEnabled)
			GPRHI_ASSERT( GPRHI_Enable(GP_RHI_RS_STENCIL_TEST) );
        else
            GPRHI_ASSERT( GPRHI_Disable(GP_RHI_RS_STENCIL_TEST) );
        _defaultState->_stencilTestEnabled = _stencilTestEnabled;
    }
	if ((_bits & RS_STENCIL_WRITE) && (_stencilWrite != _defaultState->_stencilWrite))
    {
		GPRHI_ASSERT( GPRHI_StencilMask(_stencilWrite) );
...
void RenderState::StateBlock::restore(long stateOverrideBits)
{
    ...
    {
        GPRHI_ASSERT( GPRHI_BlendFunc(GP_RHI_BLEND_ONE, GP_RHI_BLEND_ZERO) );
    ...
    {
        GPRHI_ASSERT( GPRHI_DepthMask(GP_TRUE) );
    ...
    {
		GPRHI_ASSERT( GPRHI_StencilMask(RS_ALL_ONES) );
